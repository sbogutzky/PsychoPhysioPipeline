...
for (self.report.file.name in self.report.file.names) {
  ...
  for(i in 1:nrow(self.report.data)) {
    ...
    jerk.cost.data.file.path <- paste(processed.data.directory.path, activity.directory, user.directory, date.directory, "leg-jerk-cost-data-", i, ".csv", sep="")
    kubios.hrv.data.file.path <- paste(processed.data.directory.path, activity.directory, user.directory, date.directory, "ecg-data-", i, "_hrv.txt", sep="")
