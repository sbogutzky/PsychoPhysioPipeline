...
data.file.name.1 <- readline("Type in data file name of the ecg file and press return to continue > ")
...
for (self.report.file.name in self.report.file.names) {
  ...
  for(i in 1:nrow(self.report.data)) {
    ...
    if(data.file.name.3 != "") mid.swing.data.file.path.2 <- paste(preprocessed.data.directory, activity.directory, user.directory, date.directory, data.file.name.3, "-mid-swing-indexes-", i, ".csv", sep="")
    ...
    if(file.exists(mid.swing.data.file.path.1) & file.exists(kubios.hrv.data.file.path)) {
      ...
      # Load step times
      mid.swing.data <- read.csv(mid.swing.data.file.path.1, skip = 0)
      m <- 2
      if(data.file.name.3 != "") {
        mid.swing.data.2 <- read.csv(mid.swing.data.file.path.2, skip = 0)
        mid.swing.data <- rbind(mid.swing.data, mid.swing.data.2)
        m <- 1
      }
      ...
      time.range.s <- c(round(heart.beat.times[1]) - 10, round(heart.beat.times[length(heart.beat.times)]) + 10)
      
      spm <- 60 * m / diff(step.times) 
      bpm <- 60 / diff(heart.beat.times) 
      y.lim.1 <- c(min(mean(spm) - sd(spm) * 4, mean(bpm) - sd(bpm) * 4), max(mean(spm) + sd(spm) * 4, mean(bpm) + sd(bpm) * 4))
      ...
      # SPM vs. BPM 
      plot(step.times[-1], spm, xlab = "", ylab = "Cadence & HR", xaxt = "n", xlim = time.range.s, ylim = y.lim.1, xaxs = "i", pch = 23, bg = "deepskyblue")
      points(heart.beat.times[-1], bpm, pch = 21, bg = "tomato")
      ...
      legend("topright", c("SPM", "BPM"), pch = c(23, 21), pt.bg = c("deepskyblue", "tomato"), bg = "white")
      box()
      
      title(paste(format(session.start, "%Y/%m/%d %H:%M", tz = "CET"), " #", i, sep = ""))
      
      # Stroboscopic Technique
      instantaneous.phase.data <- ComputeInstantaneousPhases(heart.beat.times, step.times)
      t <- instantaneous.phase.data$ts
      fi <- instantaneous.phase.data$fi # instantaneous phases
      psi <- (fi %% (2 * pi)) / (2 * pi) # relative phases
      
      plot(t, psi, xlab = "", ylab = expression(Psi(t)), xaxt = "n",  yaxt = "n", xlim = time.range.s, xaxs = "i", yaxs = "i", ylim = y.lim, pch = 21, bg = "darkolivegreen1")
      abline(v = seq(time.range.s[1], time.range.s[2], time.window.s), lty = "dashed", col = "grey")
      axis(1, at = seq(time.range.s[1], time.range.s[2], time.window.s), labels = seq(time.range.s[1], time.range.s[2], time.window.s), las = 1)
      ...
      indexes <- data.frame(t.s = time.points, pcoi = phase.coherence.indexes, nsei = normalized.shannon.entropy.indexes)
      indexes <- indexes[indexes$t.s > heart.beat.times[1] & indexes$t.s < heart.beat.times[length(heart.beat.times)], ]
      
      plot(indexes$t.s, indexes$nsei, type = "l", xlab = "t (s)", ylab = "Indexes", xaxt = "n",  yaxt = "n", xlim = time.range.s, xaxs = "i", yaxs = "i", ylim = y.lim, col = "deepskyblue")
      lines(indexes$t.s, indexes$pcoi, lty = 2)
      abline(v = seq(time.range.s[1], time.range.s[2], time.window.s), lty = "dashed", col = "grey")
      axis(1, at = seq(time.range.s[1], time.range.s[2], time.window.s), labels = seq(time.range.s[1], time.range.s[2], time.window.s), las = 1)
      axis(2, at = seq(y.lim[1], y.lim[2], .2), labels = seq(y.lim[1], y.lim[2], .2))
      legend("topright", c("PCoI", "NSEI"), lty = c("dashed", "solid"),  col = c("black", "deepskyblue"), bg = "white")
      box()
      ...
      write.csv(indexes, file = con, row.names = FALSE)
      close(con)
