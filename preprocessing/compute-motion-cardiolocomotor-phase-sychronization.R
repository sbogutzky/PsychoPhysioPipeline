...
for (i in 1:length(first.names)) {
  ...
  for (self.report.file.name in self.report.file.names) {
    ...
    for(j in 1:nrow(self.report.data)) {
      ...
      mid.swing.data.file.path.1 <- paste(preprocessed.data.directory, activity.directory, user.directory, date.directory, data.file.name.2, "-mid-swing-indexes-", j, ".csv", sep="")
      mid.swing.data.file.path.2 <- paste(preprocessed.data.directory, activity.directory, user.directory, date.directory, data.file.name.3, "-mid-swing-indexes-", j, ".csv", sep="")
      kubios.hrv.data.file.path <- paste(preprocessed.data.directory, activity.directory, user.directory, date.directory, data.file.name.1, "-", j, "_hrv.txt", sep="")
      ...
      if(file.exists(mid.swing.data.file.path.1) & file.exists(kubios.hrv.data.file.path)) {
        ...
        # Write csv file
        output.file.path <- paste(output.directory.path, "cls-indexes-", j, ".csv", sep = "")
        ...
        # Write csv file
        output.file.path <- paste(output.directory.path, "cls-phases-", j, ".csv", sep = "")
        op <- options(digits.secs=3)
        con <- file(output.file.path, 'w') 
        writeLines(strftime(session.start, format="%Y-%m-%d"), con = con)
        writeLines(strftime(session.start, format="%H:%M:%OS"), con = con)
