...
data.file.names <- c("imu-rn42-bc98", "imu-rn42-3b70")
activity <- "Laufen"
last.name <- "undefined"
...
optimal.cutoff.frequencies <- c(3.71, 5.14, 5.57)
    ...
    for (self.report.file.name in self.report.file.names) {
     ...
     self.report.data <- read.csv(paste(input.data.directory, self.report.file.name, sep = ""), comment.char = "#")
      ...
      for(j in 1:nrow(self.report.data)) {
        ...
        mid.swing.path <- paste(preprocessed.data.directory, activity.directory, user.directory, date.directory, data.file.name,"-mid-swing-indexes-", j,  ".csv", sep = "")
        if(file.exists(data.path) & file.exists(mid.swing.path)) {
          ...
          # Loop cycles
          for(k in 1:(length(mid.swing.indexes) - 1)) {
          
            m <- mid.swing.indexes[k]
            n <- mid.swing.indexes[(k+1)]
          ...
          print(paste(last.name, ", ", first.name, " / Jerk Cost by Cycle: ", round(jerk.cost.by.cycle.mean / 10^5, 4), sep = ""))
          print(paste(last.name, ", ", first.name, " / Jerk Cost by all Accelerations:", round(jerk.cost.by.all.accelerations / 10^5, 4), sep = ""))
          ...
          # Write csv file
          output.file.name <- paste(data.file.name, "-cycle-intervals-jerk-costs-", j, ".csv", sep = "")
