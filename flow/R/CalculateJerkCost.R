...
#' @return A vector with the jerk cost [m^2/s^5] of the acceleration.
...
CalculateJerkCost <- function(t, x, y, z, plot = F) {
  ...
  if (plot) {
    ...
    plot(t[-1], jerk.x, type = "l", xlab = "Time", ylab = expression("Jerk [" ~ m/s^3 ~ "]"), xaxs = "i", ylim=y.lim)
    lines(t[-1], jerk.y, lty = "dashed")
    lines(t[-1], jerk.z, lty = "dotted")
    plot(t[-1], jerk.x^2 + jerk.y^2 + jerk.z^2, type = "l", xlab = "Time", ylab = expression(Jerk[x]^2 ~ (t)+Jerk[y]^2 ~ (t)+Jerk[z]^2 ~ (t)), xaxs = "i")
