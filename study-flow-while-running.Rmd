...
Zur Berechung der üblichen Spektralleistung der unterschiedlichen Bänder in $ms^2$ berechne ich das Mittel der Wavelet Koeffizienten. Die daraus resultierenden \ac{HRV}-Merkmale des Laufes vom Donnerstag, den 31. Oktober 2013 bildet Tabelle \ref{tab:hrv-ergebisse} ab.
...
Die statistische Analyse der einzelnen Merkmale führte ich mit R durch. Der Effekt des Zeitpunkts der Messung von Baseline bis zur Minute 60 des Laufens evaluierte ich mit einer einfachen ANOVA bei wiederholten Messungen (ggf. mit Greenhouse-Geisser oder Huynh-Feldt Korrektur). Die Normalverteilung der Daten jeder Stichprobe prüfte zahlenmäßig mit dem Shapiro Test und visuell in QQ-Plots. Das Merkmal Besorgnis entfernte ich aus der Datensammlung, da der Läufer sie niedrig und mit mit geringer Abweichung bewertete. Einen abhängigen t-Test (Paardifferenzentest; engl. Paired t-Test) führte ich falls nötig durch, um die mittlere Differenz zweier Messabschnitte zu berechnen. P-Werte unter 0.05 sah ich als statistisch signifikant an.

```{r subset-features, echo=FALSE, message=FALSE, warning=FALSE}
  fss.features.subset <- fss.features[,c(1,3,5,7,9,11,12,15)]
  fss.features.subset[fss.features.subset[, 6] == "Running",]$measurement <- fss.features.subset[fss.features.subset[, 6] == "Running",]$measurement + 1
  fss.features.subset <- fss.features.subset[order(as.Date(fss.features.subset$activity.start,format="%Y-%m-%d %H:%M:%S")),,drop=FALSE]
  fss.features.subset$measurement <- factor(fss.features.subset$measurement, levels = c(1,2,3,4,5), labels = c("baseline", "15", "30", "45", "60"))
  
  hrv.features.subset <- hrv.features[, c(1:11, 15)  ]
  hrv.features.subset[hrv.features.subset[, 10] == "Running",]$measurement <- hrv.features.subset[hrv.features.subset[, 10] == "Running",]$measurement + 1
  hrv.features.subset <- hrv.features.subset[order(as.Date(hrv.features.subset$activity.start,format="%Y-%m-%d %H:%M:%S")),,drop=FALSE]
  hrv.features.subset$measurement <- factor(hrv.features.subset$measurement, levels = c(1,2,3,4,5), labels = c("baseline", "15", "30", "45", "60"))
...
```{r examine-features, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
  ...
  par(mfrow = c(1,2))
  for(i in 1:5) {
    plot(fss.features.subset[,i], pch=19, col=fss.features.subset$measurement, ylab = names(fss.features.subset)[i])
    boxplot(fss.features.subset[,i] ~ measurement, data=fss.features.subset)
  }

  par(mfrow = c(1,1))
  for(i in 1:5) {
    hist(fss.features.subset[,i], main = names(fss.features.subset)[i], xlab = names(fss.features.subset)[i])
    lines(density(fss.features.subset[,i]))
    rug(jitter(fss.features.subset[,i]))
  } 

  # Examine hrv features (visual)
  par(mfrow = c(1,2))
  for(i in 1:9) {
    plot(hrv.features.subset[,i], pch=19, col=hrv.features.subset$measurement, ylab = names(hrv.features.subset)[i])
    boxplot(hrv.features.subset[,i] ~ measurement, data=hrv.features.subset)
  }
  
  par(mfrow = c(1,1))
  for(i in 1:9) {
    hist(hrv.features.subset[,i], main = names(hrv.features.subset)[i], xlab = names(hrv.features.subset)[i])
    lines(density(hrv.features.subset[,i]))
    rug(jitter(hrv.features.subset[,i]))
  } 
```
  
```{r check-normality, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}

  # Check: Underlying Assumptions: Normality
  
  par(mfrow = c(2,1))
  for(i in 1:5) {
    for(j in levels(fss.features.subset$measurement)) {    
      print(paste(names(fss.features.subset[fss.features.subset$measurement == j,])[i], fss.features.subset[fss.features.subset$measurement == j,]$measurement[1]))
      print(shapiro.test(fss.features.subset[fss.features.subset$measurement == j, i]))
      qqnorm(fss.features.subset[fss.features.subset$measurement == j, i], ylab = paste(names(fss.features.subset[fss.features.subset$measurement == j,])[i], fss.features.subset[fss.features.subset$measurement == j,]$measurement[1]))
      qqline(fss.features.subset[fss.features.subset$measurement == j, i])
      plot(fss.features.subset[fss.features.subset$measurement == j, i], ylab = paste(names(fss.features.subset[fss.features.subset$measurement == j,])[i], fss.features.subset[fss.features.subset$measurement == j,]$measurement[1]))
    }
  }
  
  par(mfrow = c(2,1))
  for(i in 1:9) {
    for(j in levels(hrv.features.subset$measurement)) {    
      print(paste(names(hrv.features.subset[hrv.features.subset$measurement == j,])[i], hrv.features.subset[hrv.features.subset$measurement == j,]$measurement[1]))
      print(shapiro.test(hrv.features.subset[hrv.features.subset$measurement == j, i]))
      qqnorm(hrv.features.subset[hrv.features.subset$measurement == j, i], ylab = paste(names(hrv.features.subset[hrv.features.subset$measurement == j,])[i], hrv.features.subset[hrv.features.subset$measurement == j,]$measurement[1]))
      qqline(hrv.features.subset[hrv.features.subset$measurement == j, i])
      plot(hrv.features.subset[hrv.features.subset$measurement == j, i], ylab = paste(names(hrv.features.subset[hrv.features.subset$measurement == j,])[i], hrv.features.subset[hrv.features.subset$measurement == j,]$measurement[1]))
    }
  }
```
...
```{r repeated-anova, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
  ...
  subject <- c()
  for(i in 1:6) {
      subject       <- c(subject, rep(i, length(levels(hrv.features.subset$measurement))))
  }
  
  for(i in 1:(ncol(fss.features.subset) - 3)) {
    print(colnames(fss.features.subset)[i])
    calculateRepeatedAnova(subject, fss.features.subset$measurement, fss.features.subset[,i]) 
  }
  
  for(i in 1:(ncol(hrv.features.subset) - 3)) {
    print(colnames(hrv.features.subset)[i])
    calculateRepeatedAnova(subject, hrv.features.subset$measurement, hrv.features.subset[,i]) 
  }
```
...
Der Generalfaktor unterscheidet sich nur bei der Messung nach der 30. Minute signifikant von Baseline-Messung zu Laufmessung [$t(5) = -2.6458, p < .05$]. In den letzten drei Messungen unterscheiden sich die mittleren Differenzen des angegebenen Flusses signifikat. Der Fluss sinkt von Baseline-Messung zum Laufen. Der Effekt auf die Absorbiertheit und die \ac{AFP} ist im Vergleich von Baseline-Messung zu den vier Laufmessungen in jedem der vier Stichprobenvergleiche signikant. Beide Merkmale steigen beim Laufen an. Für alle \ac{HRV}-Merkmale, außer der Gesamtleistung [jeweils $p < .1$] und dem relativen Wert der \ac{LF} [nach der 45. Minute und nach der 60. Minute, $p < .1$], ist der Effekt für jeden Stichprobenvergleich signifikant. Es steigen die mittlere Herzrate, der relative (%) und normalisierte Wert (n. u.) der \ac{HF}. Die restlichen Merkmale fallen von Baseline-Messung zum Laufen. Dieses Ergebnis bestätigt bestehende Arbeiten über die \ac{HRV} bei sportlicher Aktivität \citep{Pichon2004, Sarmiento2013, Hoos2010}. 

#### Effekt der Messzeitpunkts auf die Merkmale
Für alle Merkmale, außer der \ac{AFP}, ist kein signifikanter Effekt zwischen den einzelnen Laufmessungen zu finden. Damit scheint der Messzeitpunkt auf diese Merkmale keinen Einfluss zu haben. Die gefühlte \ac{AFP} steigt signifikant von Messung der 15. Minute zur Messung der 30. Minute [$t(5) = -7, p < .001$] und von Messung der 30. Minute zur Messung der 45. Minute [$t(5) = -3.16, p < .05$] (siehe Abbildung \ref{fig:zeit-effekt-auf-afp}).

```{r zeit-effekt-auf-afp, echo=FALSE, warning=FALSE, dev='pdf', fig.height=5, fig.width=6.5, fig.pos='htbp', fig.cap='Effekt der Messzeitpunkts auf die AFP'}
  treatment   <- fss.features.subset$measurement
  treatments  <- levels(treatment)
  value       <- fss.features.subset[,5]
  df          <- c()
  for(i in 1:length(treatments)) 
    {
  ...
  df           <- as.data.frame(df)
  colnames(df) <- c("Baseline", "15'", "30'", "45'", "60'")
  ...
  bp <- barplot2(means, plot.ci = TRUE, ci.l = lowers, ci.u = uppers, xpd = FALSE, ylim = c(0, 7), axes = F, ylab = "Anforderungs-Fähigkeits-Passung")
  ...
  y.cord.star   <- c(y.cord[2, 1] + .08, y.cord[2, 1] + .58, y.cord[2, 1] + 1.08, 0)
