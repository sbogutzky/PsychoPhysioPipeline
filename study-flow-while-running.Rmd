...
\caption{Anordnung des Equipments; Smartphone am Oberarm; Bewegungssensoren am Schienbein und Handgelenk; EKG-Sensor mit zwei Ableitungen am Oberkörper \label{fig:anordnung-des-equipments}}
\end{figure}
...
### Datenverarbeitung
#### Flow-Kurzskala
Aus der Bewertung der \ac{FKS} berechnete ich die Faktoren der \ac{FKS}. Die Ergebnisse zeigt die Tabelle \ref{tab:fks-merkmale}. Im arithmetischen Mitte steigt die \ac{AFP} kontinuierlich Die Absorbiertheit steigt im arithmetischen Mittel bis zur 45-Minute. Danach fällt sie ein wenig. Der glatte Verlauf verhält sich gegenläufig (siehe Abbildung \ref{fig:absorbiertheit-afp-verlauf}). 
...
```{r absorbiertheit-afp-verlauf, echo=FALSE, warning=FALSE, dev='pdf', fig.height=3, fig.width=6.5, fig.pos='htbp', fig.cap='Arithmetische Mittel von Absorbiertheit, AFP und glatten Verlauf nach Ruhe, 15-, 30-, 45- und 60-Minuten Laufen [N = 6] \\label{fig:absorbiertheit-afp-verlauf}'}
  par(mfcol=c(1, 1), mar=c(3.5, 3.5, 1.5, 3.5) + 0.1, mgp=c(2, .5, 0), las=1, cex.axis=0.8, tck=.01, cex.lab=.8)

  # Plot data
  plot(tapply(fss.feature.subset[, 3], fss.feature.subset[, 7], mean), type = "b", xlab = "Messung", ylab = "", pch = 21, ylim = c(3, 6), xaxt = "n", yaxt = "n")
  lines(tapply(fss.feature.subset[, 5], fss.feature.subset[, 7], mean), type = "b", pch = 22)
  lines(tapply(fss.feature.subset[, 2], fss.feature.subset[, 7], mean), type = "b", pch = 23)
  ...
  axis(3, at = 1:length(levels(fss.feature.subset$measurement)), label = rep("", length(levels(fss.feature.subset$measurement))))
...
```{r ekg-signalverarbeitung, echo=FALSE, warning=FALSE, dev='CairoPNG', dpi=800, fig.height=4.5, fig.width=6.5, fig.pos='htbp', fig.cap='EKG-Signalverarbeitung \\label{fig:ekg-signalverarbeitung}', eval=FALSE}
  par(mfcol=c(4, 1), mar=c(3, 3, 1.5, 3) + 0.1, mgp=c(2, .5, 0), las=1, cex.axis=0.8, tck=.01, cex.lab=.8, xaxs="i", yaxs="i")

  # Load ecg data  
  ecg.data.file.path <- paste("./data/cleaned-data/running/buse-patrick/", strftime(as.POSIXct("2013-10-31 18:47:19", tz="CET"), "%Y-%m-%d--%H-%M-%S"), "/", sep="")
  ecg.data  <- read.csv(paste(ecg.data.file.path, "ecg-data.csv", sep=""), header=T)

  # Load Kubios HRV txt data
  kubios.hrv.data.path  <- paste("./data/example-data/running/buse-patrick/", strftime(as.POSIXct("2013-10-31 18:47:19", tz="CET"), "%Y-%m-%d--%H-%M-%S"), "/", sep="")
  ...
  # Set time properties
  starts    <- as.POSIXct(fss.feature.subset[,11], tz="CET")
  ends      <- as.POSIXct(fss.feature.subset[,12], tz="CET")
...
Zur Berechung der Spektralleistung der unterschiedlichen Bänder in $ms^2$ berechne ich das Mittel der Wavelet Koeffizienten. Die daraus resultierenden \ac{HRV}-Merkmale bildet Tabelle \ref{tab:hrv-merkmale} ab.
...
```{r hrv-merkmale, echo=FALSE, message=FALSE, warning=FALSE, results="asis"}
  ...
  rownames(feature.table)  <- c("Herzrate (BPM)", "LF ($ms^2$)", "HF-VHF ($ms^2$)", "Gesamt ($ms^2$)", "LF (\\%)", "HF (\\%)", "LF (n. u.)", "HF-VHF (n. u.)", "LF/HF-VHF")
...
```{r examine-features, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
  ...
  par(mfrow = c(1,2))
  for(i in 1:5) {
    plot(fss.feature.subset[,i], pch=19, col=fss.feature.subset$measurement, ylab = names(fss.feature.subset)[i])
  ...
  par(mfrow = c(1,2))
  for(i in 1:9) {
    plot(hrv.feature.subset[,i], pch=19, col=hrv.feature.subset$measurement, ylab = names(hrv.feature.subset)[i])
...
Der Generalfaktor unterscheidet sich nur bei der Messung nach der 30. Minute signifikant von der Ruhemessung zu Laufmessung [$t(5) = -2.6458, p < .05$]. In den letzten drei Messungen unterscheiden sich die mittleren Differenzen des angegebenen Flusses signifikat. Der Fluss sinkt von Ruhemessung zum Laufen. Der Effekt auf die Absorbiertheit und die \ac{AFP} ist im Vergleich von Ruhemessung zu den vier Laufmessungen in jedem der vier Stichprobenvergleiche signikant. Beide Merkmale steigen beim Laufen an. Für alle \ac{HRV}-Merkmale, außer der Gesamtleistung [jeweils $p < .1$] und dem relativen Wert der \ac{LF} [nach der 45. Minute und nach der 60. Minute, $p < .1$], ist der Effekt für jeden Stichprobenvergleich signifikant. Es steigen die mittlere Herzrate, der relative (%) und normalisierte Wert (n. u.) der \ac{HF}. Die restlichen Merkmale fallen von Ruhemessung zum Laufen. Dieses Ergebnis bestätigt bestehende Arbeiten über die \ac{HRV} bei sportlicher Aktivität \citep{Pichon2004, Sarmiento2013, Hoos2010}. 
...
<!--
Wir wollen dort Mechanismen kennenlernen, mit deren Hilfe auch das gewöhnliche eben erfreulicher gestaltet werden.

Wenn wir einmal in der Lage sind, flow operational zu definieren, haben wir die Möglichkeit, dieses Konzept als Maß des gesellschaftlichen Fortschrittes zu verwenden - ein Maß, welches die gegenwärtig geläufigen einseitigen Indikatoren materieller Leistung ergänzen würde. 
-->
...
```{r correlation-analysis, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
  ...
  for(i in I) {
    for (j in J) {
      ...
      values <- seq(min(hrv), max(hrv), 0.01)
      predicted.values <- predict(quadratic.model, list(hrv = values, hrv.2 = values^2))
      lines(values, predicted.values, col = "darkgreen", lwd = 3)
    }
  }

  # Clean up
  rm(hrv, hrv.2, I, i, J, j, predicted.values, quadratic.model, st, values)
```

<!--
  Ohne Ausreißer 11 
  Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
  flow ~ hf.power.r p < 0.01 . Hohe Werte des Gesamtfaktors korrelieren linear mit niedrigen relativen Werten der HF r = 0.39
  flow ~ lf.power.nu p < 0.01 . Hohe des Gesamtfaktors korrelieren linear mit hohen normalisierten Werten der LF r = 0.41.
  flow ~ hf.power.nu p < 0.01 . Hohe Werte des Gesamtfaktors korrelieren linear mit niedrigen normalisierten Werten der HF r = 0.41
  flow ~ lfhf p < 0.05 * Hohe Werte des Gesamtfaktors korrelieren linear mit dem hohen Werten von LF/HF r = 0.42
  
  fluency ~ hf.power.r p < 0.05 * Hohe Werte des glatten Verlaufs korrelieren linear mit niedrigen relativen Werten der HF r = 0.44
  fluency ~ hf.power.r p < 0.01 . Hohe Werte des glatten Verlaufs korrelieren mit moderaten relativen Werten der HF r2 = 0.21
  fluency ~ lf.power.nu p < 0.01 . Hohe des glatten Verlaufs korrelieren linear mit hohen normalisierten Werten der LF r = 0.41
  fluency ~ hf.power.nu p < 0.01 . Hohe Werte des glatten Verlaufs korrelieren linear mit niedrigen normalisierten Werten der HF r = 0.41
  fluency ~ lfhf p < 0.01 . Hohe Werte des glatten Verlaufs korrelieren linear mit dem hohen Werten von LF/HF r = 0.41
  
  fit ~ mean.hr p < 0.001 ***  Hohe AFP Werte korrelieren linear mit hohen Werten der mittleren Herzrate r = 0.69
  fit ~ mean.hr p < 0.01 **  Hohe AFP Werte korrelieren mit moderaten Werten der mittleren Herzrate r2 = 0.49
-->
