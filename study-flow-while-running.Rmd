...
```{r subset-leg-motion-data, echo=FALSE, message=FALSE, warning=FALSE}

  # Load ecg data 
  leg.motion.data  <- read.csv(paste(file.path, "/leg-motion-data.csv", sep=""), header=T)
  ...
  times     <- as.POSIXct(leg.motion.data[, 8]/1000, origin="1970-01-01", tz="CET")

  i <- 2
#       for(i in 1:length(starts)) {

  # Subset data
  leg.motion.data.subset     <- leg.motion.data[starts[i] <= times & ends[i] > times, ]
  
#   t     <- times[starts[i] <= times & ends[i] > times]
#   start <- t[1]
#   end   <- t[length(t)]
#   message(paste("Data from ", start, " to ", end, " (", format(round(end-start, 2), nsmall=2), ")", sep=""))
          
  # Find main frequency        
  require(TSA)
  rotation.rate.x.spectrum <- TSA::periodogram(leg.motion.data.subset[, 5], plot = T)
#   rotation.rate.y.spectrum <- TSA::periodogram(leg.motion.data.subset[, 6], plot = T)
#   rotation.rate.z.spectrum <- TSA::periodogram(leg.motion.data.subset[, 7], plot = T)
  index.max               <- which.max(rotation.rate.x.spectrum[[2]])
  main.frequency          <- rotation.rate.x.spectrum[[1]][index.max]

  # Low pass signal
  require(signal)
  butterworth.filter      <- butter(1, main.frequency * .5, "low")
  filtered.rotation.rate  <- filtfilt(butterworth.filter, leg.motion.data.subset[, 5])
  ...
  minima <- minima[m + s * .5 > filtered.rotation.rate[minima]]

#   hist(filtered.rotation.rate[minima])
#   plot(leg.motion.data.subset[, 1]/1000, leg.motion.data.subset[, 5], type="l", xlim = c(1800, 1900))
#   abline(v = leg.motion.data.subset[minima, 1]/1000)
#   plot(leg.motion.data.subset[, 1]/1000, filtered.rotation.rate, type="l", xlim = c(1800, 1900))
#   abline(v = leg.motion.data.subset[minima, 1]/1000)
#   require("zoom")
#   session.zoom()

  cycle.time      <- leg.motion.data.subset[minima, 1] / 1000
  cycle.duration  <- diff(cycle.time)

  cycle.duration  <- cycle.duration[cycle.time >= cycle.time[length(cycle.time)] - 60 * 10]
  cycle.time      <- cycle.time[cycle.time >= cycle.time[length(cycle.time)] - 60 * 10]

#   plot(cycle.time, cycle.duration, type="l")

  # Compute jerk cost per cycle
  # Isolate gravity from acceleration
  n  <- nrow(leg.motion.data.subset) 
  fs <- n / ((leg.motion.data.subset[n, 1] - leg.motion.data.subset[1, 1]) / 1000)
  butterworth.filter  <- butter(1, 1/fs * 0.8, "low")
  gravity.x           <- filtfilt(butterworth.filter, leg.motion.data.subset[, 2])
  gravity.y           <- filtfilt(butterworth.filter, leg.motion.data.subset[, 3])
  gravity.z           <- filtfilt(butterworth.filter, leg.motion.data.subset[, 4])

  # Compute linear acceleration (mit Hilfe der Rotation, die Schwerkraft errechnen)
  linear.acceleration.x <- leg.motion.data.subset[, 2] - gravity.x
  linear.acceleration.y <- leg.motion.data.subset[, 3] - gravity.y
  linear.acceleration.z <- leg.motion.data.subset[, 4] - gravity.z
  ...
  if (length(minima) > 1) {
      ...
      for(i in 1:(length(minima) - 1)) {
        ...
        t.subset                      <- leg.motion.data.subset[minima[i]:minima[i + 1], 1]
        linear.acceleration.x.subset  <- linear.acceleration.x[minima[i]:minima[i + 1]]
        linear.acceleration.y.subset  <- linear.acceleration.y[minima[i]:minima[i + 1]]
        linear.acceleration.z.subset  <- linear.acceleration.z[minima[i]:minima[i + 1]]
        jerk.cost                     <- CalculateJerkCost(t.subset, linear.acceleration.x.subset, linear.acceleration.y.subset, linear.acceleration.z.subset, plot = T)
        
#         # Normalized by cycle length
#         jerk.cost   <- jerk.cost / cycle.duration[i]
        jerk.costs  <- c(jerk.costs, jerk.cost)
      }
      jerk.costs <- c(jerk.costs, 0)
  } else {
