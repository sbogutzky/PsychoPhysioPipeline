...
### Datenverarbeitung
#### Flow-Kurzskala
Aus den Fragebogen Daten berechnete ich die Faktoren der \ac{FKS}. In Tabelle \ref{tab:fks-merkmale} sind die Ergebnisse der \ac{FKS} des Laufes vom Donnerstag, den 31. Oktober 2013 dargestellt. In dieser Sitzung z. B. steigt die \ac{AFP} kontinuierlich und beim Laufen bewertet der Läufer seine Absorbiertheit gleichbleibend im mittleren Bereich der Skala von 1 bis 7 (siehe Abbildung \ref{fig:absorbiertheit-afp-verlauf}). 
...
Ich nutze in der Studie die zweite Subdimension der \ac{FKS} zur Bewertung des subjektiven Flow-Zustands des Läufers. Laut \citet{Peifer2014} tritt Absorbiertheit nur ein, wenn Anforderungen und Fähigkeiten sich in Balance befinden — somit ist sie ausschließlich im *Flow-Kanal* anzutreffen und deshalb ein repräsentativer Indikator für Flow als der Generalfaktor, der sich aus den zwei Subdimension zusammensetzt. Den Beleg hierfür geben \citet[S. 69]{Rheinberg2003a} in ihrer Studie. Sie stellten fest, dass Unterforderung keinen Einfluss auf den glatten Verlauf hat, jedoch die Absorbiertheit negativ beeinflusst. Dies ist auch in der Abbildung \ref{fig:absorbiertheit-afp-verlauf} im Vergleich von Baseline (erste Messung) und Lauf zu erkennen. 
...
#### HRV-Analyse
Von den \ac{EKG}-Daten entfernte ich die Zeiten für die Befragung und erhielt jeweils 15-Minuten Abschnitte (siehe Abbildung \ref{fig:ekg-signalverarbeitung} von A zu B). Zur schnelleren Bearbeitung teilte ich die 15-Minuten Abschnitte in jeweils drei fünf Minuten Abschnitte (siehe Abbildung \ref{fig:ekg-signalverarbeitung} C). Zur RR-Interval Berechnung las ich die 90 fünf Minuten Abschnitte in Kubios \ac{HRV} (Version 2.1) ein. Kubios \ac{HRV} automatische R-Spitzen-Erkennung erkennt die meisten Herzschläge. Trotzdem ist eine manuelle Nachbearbeitung notwendig. Nicht erkannte Herzschläge fügte ich hinzu und zuviel erkannte Herzschläge entfernte ich. \ac{EKG}-Daten mit mehr als 2% an Artefakten habe ich aus der Datensammlung entfernt. 
Zur \ac{HRV}-Analyse nutze ich nur die letzten fünf Minuten jedes 15-Minuten Abschnittes. Das hatte folgende Gründe:
...
```{r baseline-laufen-vergleich, echo=FALSE, message=FALSE, warning=FALSE, results="asis", eval=FALSE}
  ...
  fss.features <- features[,1:6]
  hrv.features <- features[, c(1, 7:15)]

  plot(fss.features, main="FSS Feature Data", pch=19, col=fss.features$m)
  plot(hrv.features, main="HRV Feature Data", pch=19, col=hrv.features$m)

  par(mfrow = c(1,2))
  for(i in 2:15) {
    plot(features[features$m !=1,i], pch=19, col=features[features$m !=1,]$m)
    boxplot(features[features$m !=1,i] ~ m, data=features[features$m !=1,], main=names(features[features$m !=1,])[i])
    }
