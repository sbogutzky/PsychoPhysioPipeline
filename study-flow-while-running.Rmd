...
```{r examine-features, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
  ...
  for(i in 1:5) {
    print(names(fss.features.subset[fss.features.subset[, 6] == "Running",])[i])
    print(shapiro.test(fss.features.subset[fss.features.subset[, 6] == "Running",i]))
    qqnorm(fss.features.subset[fss.features.subset[, 6] == "Running",i], ylab = names(fss.features.subset[fss.features.subset[, 6] == "Running",])[i])
    qqline(fss.features.subset[fss.features.subset[, 6] == "Running",i])
  }

  for(i in 1:9) {
    print(names(hrv.features.subset[hrv.features.subset[, 10] == "Running",])[i])
    print(shapiro.test(hrv.features.subset[hrv.features.subset[, 10] == "Running",i]))
    qqnorm(hrv.features.subset[hrv.features.subset[, 10] == "Running",i], ylab = names(hrv.features.subset[hrv.features.subset[, 10] == "Running",])[i])
    qqline(hrv.features.subset[hrv.features.subset[, 10] == "Running",i])
  }


  # 

  flow.baseline <- fss.features.subset[fss.features.subset[,8] == 1 & fss.features.subset[, 6] == "Baseline", 3]
  flow.running  <- fss.features.subset[fss.features.subset[,8] == 1 & fss.features.subset[, 6] == "Running", 3]

  x.mean.1 <- mean(flow.baseline)
  x.mean.2 <- mean(flow.running)

  s.1 <- var(flow.baseline)
  s.2 <- var(flow.running)
  
  n.1 <- length(flow.baseline)
  n.2 <- length(flow.running)

  t <- (x.mean.1 - x.mean.2) / sqrt(s.1/n.1 + s.2/n.2)
  ...
  t.test(flow.baseline, flow.running)
