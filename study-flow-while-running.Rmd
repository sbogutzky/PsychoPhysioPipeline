...
```{r baseline-laufen-vergleich, echo=FALSE, message=FALSE, warning=FALSE, results="asis"}
  ...
  for (i in 2:ncol(features)) {
    ...
    # Get p value
    #fit <- Anova(lm(value ~ m, data=features.stack, contrasts=list(topic=contr.sum, sys=contr.sum)), type=3)
    fit <- anova(aov(value ~ m, data=features.stack))
...
```{r lauf-messzeitpunkt-vergleich, echo=FALSE, message=FALSE, warning=FALSE, results="asis"}
  ...
  for (i in 2:ncol(features)) {
    col.name <- colnames(features)[i]
    boxplot(features[,i]  ~ m, data = features, main = col.name)
    print(col.name)
  ...
  # Clean up
  rm(features, features.stack, means, row, sds, col.name fit, i, tukey)
```

## Unterschied zwischen den Laufmessungen

Ich testete auch, ob zwischen den erhobenen Merkmalen der einzelnen Laufmessungen ein Unterschied besteht. Bei den Flow-Merkmalen hat der Zeitpunkt nur Einfluss auf das Merkmal der \ac{AFP}. Die \ac{AFP} steigt von Messung zu Messung (siehe Abbildung). Für die HRV-Merkmalen ist ein signifikanter Unterschied zwischen den Laufmessungen bei den Merkmalen der mittlere Herzrate, LF ($ms^2$), Total ($ms^2$), LF (\%), LF (n. u.) und LF (n. u.) und LF/HF zu erkennen. Um festzustellen wo, die Unterschiede liegen führte ich eine
