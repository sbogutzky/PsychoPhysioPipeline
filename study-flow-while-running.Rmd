...
library(RHRV)
for (j in 1:i) {
  hrv.file.name <- paste(file.path, "/ecg-data-", i, "_hrv.txt", sep="")
  if(file.exists(hrv.file.name)) {
      ...
      hrv.data          <- InterpolateNIHR(hrv.data, method="spline")
      PlotHR(hrv.data)
...
par(mfcol=c(2, 1), mar=c(2.5, 2.5, 1.5, 3.5) + 0.1, mgp=c(1.5, .5, 0), las=1, cex.axis=0.8, tck=.03, cex.lab=.8, xaxs="i", yaxs="i")
