...
## Datenverarbeitung
### Flow-Kurzskala
Aus der Bewertung der \ac{FKS} berechnete ich die Faktoren der \ac{FKS}. Die Ergebnisse zeigt die Tabelle \ref{tab:fks-merkmale}. Der Gesamtfaktor steigt im arithmetischen Mittel von Ruhemessung zur ersten Laufmessung. Danach sinkt er ein wenig. Der glatte Verlauf fällt im arithmetischen Mittel bis zur 45-Minute. Danach steigt er ein wenig. Die Absorbiertheit verhält sich gegenläufig. Im arithmetischen Mitte steigt die \ac{AFP} kontinuierlich (siehe Abbildung \ref{fig:fks-faktoren}). 
...
```{r fks-faktoren, echo=FALSE, warning=FALSE, dev='pdf', fig.height=3, fig.width=6.5, fig.pos='htbp', fig.cap='Arithmetische Mittel von Absorbiertheit, AFP und glatten Verlauf nach Ruhe, 15-, 30-, 45- und 60-Minuten Laufen [N = 6] \\label{fig:fks-merkmale}'}
  par(mfcol=c(1, 1), mar=c(3.5, 3.5, 1.5, 3.5) + 0.1, mgp=c(2, .5, 0), las=1, cex.axis=0.8, tck=.01, cex.lab=.8)

  # Plot data
  plot(tapply(fss.feature.subset[, 1], fss.feature.subset[, 7], mean), type = "b", xlab = "Messung", ylab = "", pch = 21, ylim = c(3, 6), xaxt = "n", yaxt = "n")
  lines(tapply(fss.feature.subset[, 2], fss.feature.subset[, 7], mean), type = "b", pch = 22)
  lines(tapply(fss.feature.subset[, 3], fss.feature.subset[, 7], mean), type = "b", pch = 23)
  lines(tapply(fss.feature.subset[, 5], fss.feature.subset[, 7], mean), type = "b", pch = 24)
  ...
  legend("bottomright", inset=.05, c("Gesamtfaktor", "Glatter Verlauf", "Absorbiertheit", "AFP"), pch = c(21:24), cex = .7)
...
### Korrelationsanalyse
Um mögliche Zusammenhänge zwischen den Faktoren der \ac{FKS} und der Merkmale der \ac{HRV} beim Laufen aufzuklären, berechnete ich den bivariate Korrelationskoeffizient nach Pearson für alle Kombination von \ac{FKS}-Faktoren und \ac{HRV}-Mermalen. Die Normalverteilung verifizierte ich mit dem Shapiro Test und QQ-Plots. Gleichzeitig prüfte ich die Beziehung zweier Werte im Streudigramm. Da dieses Vorgehen nur auf lineare Zusammenhänge testet, ermittelte ich zusätzlich wie \citet{Peifer2014} einen quadratischen Zusammenhang. Für alle Zusammenhänge testete R die statistische Signifikanz. P-Werte unter 0.05 sah ich als statistisch signifikant an.  
...
#   hrv.feature.subset.running$lfhf         <- lfhf 
  ...
  # Clean up
  rm(hrv, hrv.2, i, j, corr, predicted.values, quadratic.model, st, values)
```

# Ergebnis

## Effekt der Aktivität Laufen auf die Merkmale
Der Generalfaktor unterscheidet sich nur bei der Messung nach der 30. Minute signifikant von der Ruhemessung zu Laufmessung [$t(5) = -2.6458, p < .05$]. In den letzten drei Messungen unterscheiden sich die mittleren Differenzen des angegebene Verlaufs signifikat. Der glatte Verlauf sinkt von Ruhemessung zum Laufen. Der Effekt auf die Absorbiertheit und die \ac{AFP} ist im Vergleich von Ruhemessung zu den vier Laufmessungen in jedem der vier Stichprobenvergleiche signikant. Beide Merkmale steigen beim Laufen an. Für alle \ac{HRV}-Merkmale, außer der Gesamtleistung [jeweils $p < .1$] und dem relativen Wert der \ac{LF} [nach der 45. Minute und nach der 60. Minute, $p < .1$], ist der Effekt für jeden Stichprobenvergleich signifikant. Es steigen die mittlere Herzfrequenz, der relative (%) und normalisierte Wert (n. u.) der \ac{HF}. Die restlichen Merkmale fallen von Ruhemessung zum Laufen. Dieses Ergebnis bestätigt bestehende Arbeiten über die \ac{HRV} bei körperlicher bzw. sportlicher Belastung \citep{Pichon2004, Sarmiento2013, Hoos2010}. 
...
Der Shapiro Test ergab für die Besorgnis, für die absoluten Werte der \ac{LF} und \ac{HF} und dem relativen Wert des \ac{LF} keine Normalverteilung. Für diese Faktoren und  Merkmale führte ich keine Korrelationsanalyse durch. Bei ersten Observation der Streudigramme fand ich einen Ausreißer in der Bewertung der \ac{FKS} beim dritten Lauf bei Minute 45. Nach der Begutachtung der Bewegungsdaten stellte ich fest, dass der Läufer vor der 45 Minute eine kurze Pause eingelegt hatte. Aus diesem Grund entfernte ich den gesamten Datensatz aus der Datensammlung. 

Mit unter 20% Varianzaufklärung beobachtete ich für den linearen Zusammenhang eine schlechte Vorhersagbarkeit des Gesamtfaktors durch die \ac{HRV}-Merkmalen \ac{HF} (%), \ac{LF} (n.u.), \ac{HF} (n.u.) oder LF/HF. Das Verhältnis von \ac{LF} und \ac{HF} mit der größten Varianzaufklärung von 18% lag unter dem Signifikanzneviau von $p < 0.05$. Die anderen drei \ac{HRV}-Merkmalen lagen jeweils bei $p < 0.1$. Alle weiteren Zusammenhänge hatten ein Signifikanzniviau von weit über $p < 0.1$. Daraus ergibt sich, dass hohe Werte der der relativen \ac{HF} und normalisieren \ac{HF} mit niedrigen Werten des Gesamtenfaktors in Zusammenhang stehen. Der Zusammenhang von Gesamtfaktor und normalisierte \ac{LF} oder dem Verhältnis von \ac{LF} und \ac{HF} verhält sich anders herum. Die Vorhersagbarkeit durch einen linearen Zusammenhang des glatten Verlauf und der \ac{HRV}-Merkmale der \ac{HF} (%), \ac{LF} (n.u.), \ac{HF} (n.u.) oder LF/HF ist sehr ähnlich. Einen nicht zufälliger systematischen Zusammenhang von Absorbiertheit und einem \ac{HRV}-Merkmal fand ich nicht.

Ein Indiz das der Läufer die \ac{FKS} sehr gewissenhaft ausgefüllt hat, zeigt der Zusammenhang von \ac{AFP} und mittlerer Herzfrequenz. Mit 47% Varianzaufklärung beobachte ich eine mittlere Vorhersagbarkeit. Es besteht ein nicht zufälliger systematischen Zusammenhang zwischen der \ac{AFP} und der mittlere Herzfrequenz [$p < 0.001$].
Daraufhin kann geschlossen werden, je größer die \ac{AFP} desto größer die mittlere Herzfrequenz.
