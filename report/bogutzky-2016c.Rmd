...
# Set paths
root.path <- "/Volumes/flow/Documents/eigene-dateien/bogutzky-simon/daten/2013/"
...
# Create data frame
data.1 <- rbind(data.1, data.frame(subject = subjects, session = sessions, measurement = measurements, absorption = fss.features[, 3], mean.hr = hrv.features[, 3], mean.stride = 60000 / stride.features[, 1], mean.nsei = cls.features[, 1]))
...
```{r variables-stop-wise, echo=FALSE, warning=FALSE, results="asis"}
  ...
  colnames(feature.table) <- c(levels(data.2[, 3]), "Total")
  rownames(feature.table) <- c("Absorption", "Mean HR (1/min)", "Mean Stride (1/min)", "Mean Norm. Shannon Entropy Index")
...
plot(data.2[, 5], data.2[, 4], xlab = "Mean HR (1/min)", ylab = "Absorption")
plot(data.2[, 6], data.2[, 4], xlab = "Mean Stride (1/min)", ylab = "Absorption")
plot(data.2[, 7], data.2[, 4], xlab = "Mean Norm. Shannon Entropy Index", ylab = "Absorption")
```
...
plot(data.3[, 5], data.3[, 4], xlab = "Mean HR (1/min)", ylab = "Absorption")
plot(data.3[, 6], data.3[, 4], xlab = "Mean Stride (1/min)", ylab = "Absorption")
plot(data.3[, 7], data.3[, 4], xlab = "Mean Norm. Shannon Entropy Index", ylab = "Absorption")
```
...
hist(data.3[, 4], main = "Absorption", xlab = "Absorption")
print("Absorption")
shapiro.test(data.3[, 4])

hist(data.3[, 5], main = "Mean HR", xlab = "Mean HR (1/min)")
print("Mean HR")
shapiro.test(data.3[, 5])

hist(data.3[, 6], main = "Mean Stride", xlab = "Mean Stride (1/min)")
print("Mean Stride")
shapiro.test(data.3[, 6])

hist(data.3[, 7], main = "Mean Norm. Shannon Entropy Index", xlab = "Mean Norm. Shannon Entropy Index")
...
cor.test(data.3[, 5], data.3[, 4], "greater", "pearson")
cor.test(data.3[, 7], data.3[, 4], "greater", "pearson")
cor.test(data.3[, 6], data.3[, 4], "less", "pearson")
```
...
for(level in levels(data.1[, 2])) {
  ...
  points(data.1[data.1[, 2] == level, 3], (data.1[data.1[, 2] == level, 5] / 190) * 7, type = "b", col = "red")
  points(data.1[data.1[, 2] == level, 3], (data.1[data.1[, 2] == level, 6] / 90) * 7, type = "b", col = "blue")
  points(data.1[data.1[, 2] == level, 3], (data.1[data.1[, 2] == level, 7] / 0.1) * 7, type = "b", col = "green")
}
