...
for (i in 1:nrow(hrv.baseline.features)) {
  for (j in 1:4) {
    hrv.baseline.features.meanhr <- c(hrv.baseline.features.meanhr, 0)
    hrv.baseline.features.rmssd  <- c(hrv.baseline.features.rmssd, 0)
  }
}
...
for (i in 1:nrow(fss.features)) {
  ...
  if(file.exists(paste(directory.path, file.name, sep = ""))) {
    ...
    data[, 3]   <- data[, 3] / 10^4
    anomaly  <- DetectAnomaly(data[, 2], data[, 3], expression("Cycle Interval ("~s~")"), expression("JC (x"~10^4~m^2*s^{-5}~")"), c(.4, 1.3), c(0, 6), epsilon = epsilon) 
    if(anomaly$epsilon != 0 & length(anomaly$outliers) > 0) {
      data        <- data[-anomaly$outliers, ]
      epsilon     <- anomaly$epsilon
    }
...
# Remove unused stuff
rm(additional.features, data, feature.data.frame, activity.start, anomaly, col.names, date.directory, directory.path, epsilon, estimated.features, feature.vector, file.name, measurement,i, j, range.intervals)
```
...
jerk.cost.expression  <- expression("Mean JC (x"~10^4~m^2*s^{-5}~")")
...
x <- hrv.features$meanhr[breaks == 3] - hrv.baseline.features.meanhr
y <- fss.features$absorption
PlotLinearRegression(x, y, xlab = meanhr.expression, ylab = absorption.expression)

x <- hrv.features$rmssd[breaks == 3] - hrv.baseline.features.rmssd
y <- fss.features$absorption
PlotLinearRegression(x, y, ylab = absorption.expression, xlab = rmssd.expression)

x <- jc.features$mean.jerk.cost[breaks == 3]
y <- fss.features$absorption
PlotLinearRegression(x, y, ylab = absorption.expression, xlab = jerk.cost.expression)

x <- jc.features$mean.jerk.cost[breaks == 3]
y <- hrv.features$meanhr[breaks == 3] - hrv.baseline.features.meanhr
PlotLinearRegression(x, y, ylab = meanhr.expression, xlab = jerk.cost.expression)
...
x <- cls.features$mean.nsei[breaks == 3]
y <- fss.features$absorption
PlotLinearRegression(x, y, ylab = absorption.expression, xlab = nsei.expression)

x <- hrv.features$rmssd[breaks == 3] - hrv.baseline.features.rmssd
y <- cls.features$mean.nsei[breaks == 3]
PlotLinearRegression(x, y, ylab = rmssd.expression, xlab = nsei.expression)

x <- cls.features$mean.nsei[breaks == 3]
y <- jc.features$mean.jerk.cost[breaks == 3] 
PlotLinearRegression(x, y, ylab = jerk.cost.expression, xlab = nsei.expression)

x <- cls.features$mean.nsei[breaks == 3]
y <- cls.features$mean.pcoi[breaks == 3]
PlotLinearRegression(x, y, ylab = pcoi.expression, xlab = nsei.expression)
```
...
for (i in sessions) {
  ...
  abline(v = ticks, lty = "dashed", col = "darkgrey")
  box()
  plot(data$t.s, runmean(data$jerk.cost.m2s5/10^4, 10, endrule = "NA"), xlab = "", ylab = expression("JC (x"~10^4~m^2*s^{-5}~")"), xlim = time.range, pch = 21, xaxt = "n")
  axis(1, at = ticks)
  abline(v = ticks, lty = "dashed", col = "darkgrey")
  box()
}
