...
for (activity in activities) {
  for (user in users) {
    ...
    data.1b <- data.frame(subject = subjects, date = dates, fss.features[, 1:5])
    rm(fss.feature.path)
...
for(i in unique(as.numeric(data.1$subject))) {
  data.1.1 <- data.1[as.numeric(data.1$subject) == 1, ]
  ...
  PlotModels(data.1.1$meanhr, data.1.1$absorption, x.lim = c(150, 200), sub.line = 1.3, interval = 1)
  print("-----------------------------------------")
  ...
  PlotModels(data.1.1$mean.jerk.cost, data.1.1$absorption, x.lim = c(1, 5), sub.line = 1.3, interval = .1)
  print("-----------------------------------------")
  ...
  PlotModels(data.1.1$mean.cycle.interval, data.1.1$absorption, x.lim = c(.65, 7), sub.line = 1.3, interval = .0001)
  print("-----------------------------------------")
...
source("./code-snippets/functions.r")
data.5 <- data.1
data.5 <- data.5[-c(5, 18), ]

par("mfcol" = c(3, 1), mar = c(3, 3, 2.5, 1) + 0.1, mgp = c(2, .5, 0), las = 1, cex.axis = 0.8, tck = .03, cex.lab = .8, xaxs = "r", yaxs = "r")

for(i in unique(as.numeric(data.5$subject))) {
  data.5.1 <- data.5[as.numeric(data.5$subject) == i, ]
  plot(data.5.1$meanhr, data.5.1$mean.nsei, xlab = "Mean HR", ylab = "Cardio-locomotor phase synchronisation", pch = 21, bg = colors[1])
  PlotModels(data.5.1$meanhr, data.5.1$mean.nsei, x.lim = c(100, 200), sub.line = 1.25, interval = 1)
  
  plot(data.5.1$meanhr, data.5.1$absorption, xlab = "Mean HR", ylab = "Absorption", pch = 21, bg = colors[1])
  PlotModels(data.5.1$meanhr, data.5.1$absorption, x.lim = c(100, 200), sub.line = 1.25, interval = 1)
  
  plot(data.5.1$mean.nsei, data.5.1$absorption, xlab = "Cardio-locomotor phase synchronisation", ylab = "Absorption", pch = 21, bg = colors[1])
  PlotModels(data.5.1$mean.nsei, data.5.1$absorption, x.lim = c(0, 1), sub.line = 1.25, interval = .01)
}
rm(data.5.1)
```
