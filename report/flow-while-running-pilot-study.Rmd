...
data.1.pre <- rbind(data.1.pre, data.frame(subject = subjects, date = dates, fss.features[, c(1, 3, 5, 7, 9:11)]))
rm(fss.feature.path)
...
# Create data frame
data.1 <- rbind(data.1, data.frame(subject = subjects, date = dates, measurement = measurements, fss.features[, c(1, 3, 5, 7, 9:11)], hrv.features[, c(3, 5)], cls.features[, 1:2], jc.features[, 1:2]))
    
rownames(data.1) <- seq(length = nrow(data.1))

rm(user, activity, fss.features, hrv.features, jc.features, cls.features, subjects, dates, measurements)
```

```{r course-of-flow}
library(lme4, lattice)

random.intercept.model <- lmer(flow ~ mean.nsei + (1 | date), data = data.1, REML = FALSE)
random.intercept.model <- update(random.intercept.model, subset = !is.na(mean.nsei))
# fixef(random.intercept.model)
# ranef(random.intercept.model)

null.model <- update(random.intercept.model,	~	.	-mean.nsei)
# fixef(null.model)
# ranef(null.model)

anova(null.model, random.intercept.model)

pred.flow <- fitted(random.intercept.model)

data.2 <- unique(data.frame(cbind(pred.flow = pred.flow, mean.nsei = mean.nsei[!is.na(mean.nsei)], date = date[!is.na(mean.nsei)])))

xyplot(pred.flow ~ mean.nsei, data = data.2, groups = date, type = c("p", "l"), col = 3)
```
...
```{r Zeiteffekt}
parameter <- data.1$mean.hr

par(mfrow = c(1, 1))
hist(parameter)
print(shapiro.test(parameter))

flow.daten <- data.frame(parameter = parameter, messzeitpunkt = factor(rep(c("15'", "30'", "45'", "60'"), 6)))

flow.mittelwerte <- tapply(flow.daten$parameter, flow.daten$messzeitpunkt, mean, na.rm=TRUE)
flow.mittelwerte
flow.standardabweichung <- tapply(flow.daten$parameter, flow.daten$messzeitpunkt, sd, na.rm=TRUE)

x.werte <- barplot(flow.mittelwerte, las = 2)
arrows(x.werte, flow.mittelwerte - flow.standardabweichung, x.werte, flow.mittelwerte + flow.standardabweichung, angle = 90, code = 3)

plot(flow.daten$parameter ~ flow.daten$messzeitpunkt, las = 2, xlab = "", col = 2)

flow.aov <- aov(parameter ~ messzeitpunkt, data = flow.daten)
summary(flow.aov)
...
levene(flow.daten$parameter, flow.daten$messzeitpunkt)
summary.lm(flow.aov)

pairwise.t.test(flow.daten$parameter, flow.daten$messzeitpunkt, p.adj = "holm")

par(mfrow = c(1, 1))
plot(data.1$mean.nsei[-c(1:4, 17, 21)], data.1$flow[-c(1:4, 17, 21)], col = flow.daten$messzeitpunkt[-c(1:4, 17, 21)])

modl <- lm(data.1$flow[-c(1:4, 17, 21)] ~ data.1$mean.nsei[-c(1:4, 17, 21)])
summary(modl)
abline(modl)
