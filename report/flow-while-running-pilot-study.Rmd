...
```{r variables-session-wise, echo=FALSE, warning=FALSE, results="asis"}
  ...
  rownames(feature.table) <- c("Generalfaktor", "Glatter Verlauf", "Absorbiertheit", "Herzfrequenz ($BPM$)", "RMSSD ($ms$)", "Norm. Shan. Entr. Index", "Bew. ($\\times 10^5 \\: m^2 \\cdot s^{-5}$)")
  
  # Print latex table
  table                   <- xtable(feature.table, label = "tab:indikatoren-nach-sitzung", align = "lyyyyyyy", caption = "Variabeln: Arithmetisches Mittel ± Standardabweichung zu den sechs Sitzungen [N = 3] \\emph{Anmerkung}: Bew. = Bewegungsaufwand.") 
...
```{r variables-measurement-wise, echo=FALSE, warning=FALSE, results="asis"}
  ...
  # Print latex table
  table                   <- xtable(feature.table, label = "tab:variabeln-nach-messzeitpunkt", align = "lyyyy", caption = "Variabeln: Arithmetisches Mittel ± Standardabweichung zu den drei Messzeitpunkten [N = 6] \\emph{Anmerkung}: Bew. = Bewegungsaufwand.") 
  print(table, caption.placement = "top", sanitize.rownames.function = function(x){x}, comment = FALSE, booktabs = TRUE)
...
rownames(cor.table) <- c("1. Generalfaktor", "2. Glatter Verlauf", "3. Absorbiertheit", "4. Herzfrequenz ($BPM$)", "5. RMSSD ($ms$)", " 6. Norm. Shan. Entr. Index", "7. Bew. ($\\times 10^5 \\: m^2 \\cdot s^{-5}$)")
...
plot(data.3$mean.nsei, data.3$absorption, xlab = "Normalisierter Shannon Entropie Index", ylab = "Absorbiertheit", pch = 24, bg = rgb(96/255, 65/255, 79/255))
...
plot(data.3$mean.jerk.cost, data.3$fluency, xlab = "Bewegungsaufwand ($\\times 10^5 \\: m^2 \\cdot s^{-5}$)", ylab = "Glatter Verlauf", pch = 23, bg = rgb(0/255, 152/255, 199/255))
...
\begin{figure}
	...
	\caption{Obversationen. (A) Absorbiertheit und mittlerer HR; (B) Absorbiertheit und RMSSD; (C) Glatterverlauf und mittlerer Bewegungsaufwand; (D) Absorbiertheit und mittlerer normalisierter Shannon Entropie Index. \emph{Anmerkung}: Durchgezogene Linie stellt das bestmögliche liniare Modell dar. Gestrichelte Linie stellt ggf. das bestmögliche quardratische Modell dar. Quelle: Eigene Darstellung}
...
grid(col = rgb(186/255, 187/255, 194/255))
box()
plot(jc.data$t.s[jc.data$t.s > n & jc.data$t.s < m], jc.data$jerk.cost.m2s5[jc.data$t.s > n & jc.data$t.s < m] / 10^5, xlab = "Zeit ($s$)", ylab = "Bew. ($\\times 10^5 \\: m^2 \\cdot s^{-5}$)", xaxs = "i", yaxs = "i", ylim = c(.1, .2)  , pch = 23, bg = rgb(0/255, 152/255, 199/255))
grid(col = rgb(186/255, 187/255, 194/255))
box()
...
grid(col = rgb(186/255, 187/255, 194/255))
box()
title(sub = "Ueberforderung - Absorbiertheit: 3.75")
...
\begin{sidewaysfigure}
	...
	\caption{Drei Minuten Daten aus einem Lauf mit Unterforderung (links), einem optimalen Lauf (mitte) und einem Lauf mit Überforderung (rechts) \emph{Anmerkung}: Bew. = Bewegungsaufwand. Quelle: Eigene Darstellung}
...
\begin{figure}
	...
	\caption{Obversationen. Mittlerer normalisierter Shannon Entropie Index und mittlerer HR \emph{Anmerkung}: Durchgezogene Linie stellt das bestmögliche liniare Modell dar. Gestrichelte Linie stellt ggf. das bestmögliche quardratische Modell dar. Quelle: Eigene Darstellung}
...
# Plot fluency on jerk cost
plot(data.4$mean.jerk.cost, data.4$fluency, xlab = "Bewegungsaufwand ($\\times 10^5 \\: m^2 \\cdot s^{-5}$)", ylab = "Glatter Verlauf", pch = 23, bg = rgb(0/255, 152/255, 199/255))
