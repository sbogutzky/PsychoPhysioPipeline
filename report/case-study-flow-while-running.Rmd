...
# Setup tikz device
tikz("./tikz/5_2_daten.tex", width = 7.91, height = 5.22)
...
tikz("./tikz/5_5_grundlage_der_klps.tex", width = 5.22, height = 5.22)
...
# Print latex table
table <- xtable(data.table.flow, label = "tab:generalfaktor_fallstudie_laufen", align = "lrrr", caption = "Arithmetisches Mittel, Standardabweichung und Item-Faktor-Korrelation der Items des Generalfaktors der ersten Studie zum Flow-Erleben beim Laufen") 
print(table, caption.placement = "top", sanitize.rownames.function = function(x){x}, comment = FALSE, booktabs = TRUE)

# Print latex table
table <- xtable(data.table.fluency, label = "tab:glatter_verlauf_fallstudie-laufen", align = "lrrr", caption = "Arithmetisches Mittel, Standardabweichung und Item-Faktor-Korrelation der Items des glatten Verlaufs der ersten Studie zum Flow-Erleben beim Laufen [$N = 24$]") 
print(table, caption.placement = "top", sanitize.rownames.function = function(x){x}, comment = FALSE, booktabs = TRUE)
  
# Print latex table
table <- xtable(data.table.absorption, label = "tab:absorbiertheit_fallstudie_laufen", align = "lrrr", caption = "Arithmetisches Mittel, Standardabweichung und Item-Faktor-Korrelation der Items der Absorbiertheit der ersten Studie zum Flow-Erleben beim Laufen [$N = 24$]") 
print(table, caption.placement = "top", sanitize.rownames.function = function(x){x}, comment = FALSE, booktabs = TRUE)
...
# Setup tikz device
tikz("./tikz/5_6_ubersicht_nach_laufen_erste_studie.tex", width = 5.22, height = 5.22)
...
plotBoxAndWhiskerPlot(data.1$session, data.1$mean.jerk.cost, col = rgb(0/255, 152/255, 199/255, .5), ylim = c(14, 24), main = "Doppelschrittfr.", ylab = "$1/min$", pch = 16)
...
plotBoxAndWhiskerPlot(data.1$session, data.1$mean.nsei, col = rgb(0/255, 152/255, 199/255, .5), ylim = c(0, .5), main = "Norm. Shan. Entr. In.", ylab = "$[0, 1]$", pch = 16)
...
```{r variables-session-wise, echo=FALSE, warning=FALSE, results="asis"}
  ...
  # Print latex table
  table                   <- xtable(feature.table, label = "tab:ubersicht_nach_laufen_erste_studie", align = "lyyyyyyy", caption = "Übersicht der expliziten und impliziten Merkmale nach Läufen der ersten Studie: Arithmetisches Mittel $\\pm$ Standardabweichung zu den sechs Läufen [$N = 4$] -+- \\emph{Anmerkung}: Bew. = Bewegungsaufwand.") 
...
# Setup tikz device
tikz("./tikz/5_7_ubersicht_nach_messzeitpunkten_erste_studie.tex", width = 5.22, height = 5.22)
...
plotBoxAndWhiskerPlot(data.1$measurement, data.1$mean.jerk.cost, col = rgb(0/255, 152/255, 199/255, .5), ylim = c(14, 24), main = "Doppelschrittfr.", ylab = "$1/min$", pch = 16)
...
plotBoxAndWhiskerPlot(data.1$measurement, data.1$mean.nsei, col = rgb(0/255, 152/255, 199/255, .5), ylim = c(0, .5), main = "Norm. Shan. Entr. In.", ylab = "$[0, 1]$", pch = 16)
...
```{r variables-measurement-wise, echo=FALSE, warning=FALSE, results="asis"}
  ...
  # Print latex table
  table                   <- xtable(feature.table, label = "tab:ubersicht_nach_messzeitpunkten_erste_studie", align = "lyyyyy", caption = "Übersicht der expliziten und impliziten Merkmale nach Messzeitpunkten der ersten Studie: Arithmetisches Mittel $\\pm$ Standardabweichung zu den sechs Läufen [$N = 6$] -+- \\emph{Anmerkung}: Bew. = Bewegungsaufwand.") 
  print(table, caption.placement = "top", sanitize.rownames.function = function(x){x}, comment = FALSE, booktabs = TRUE)
...
wilcoxsign_test(data.1[as.numeric(data.1[, 3]) == 3, 10] ~ data.1[as.numeric(data.1[, 3]) == 4, 10], distribution="exact")
...
# Print latex table
table <- xtable(cor.table, label = "tab:korrelationen_erste_studie_laufen", align = "lxxxxxxxxxx", caption = "Korrelationsmatrix der ersten Studie zum Flow-Erleben beim Laufen: Arithmetisches Mittel, Standardabweichung und Korrelationen [$N = 24$] -+-Anmerkung: Bew. = Bewegungsaufwand. -+-* Korrelation ist auf dem Niveau von 0,05 (zweiseitig) signifikant. -+-** Korrelation ist auf dem Niveau von 0,01 (zweiseitig) signifikant.") 
print(table, floating.environment="sidewaystable", caption.placement = "top", sanitize.rownames.function = function(x){x}, comment = FALSE, booktabs = TRUE)
...
color <- rep(rgb(0/255, 152/255, 199/255), 24)
color[data.1$mean.ms.interval < 87] <- rgb(96/255, 65/255, 79/255)
color[data.1$mean.ms.interva > 88] <- rgb(229/255, 66/255, 66/255)

pchs <- rep(21, 24)
pchs[data.1$mean.ms.interva < 87] <- 22
pchs[data.1$mean.ms.interva > 88] <- 23

plotRelation(data.1$mean.ms.interval, data.1$flow, xlab = "Doppelschrittfrequenz ($1/min$)", ylab = "Generalfaktor", pch = pchs, bg = color)
grid(col = rgb(186/255, 187/255, 194/255))
box()

plot(data.1$mean.ms.interval, data.1$mean.jerk.cost, ylab = "Bew. ($\\times 10^3 \\: m^2 \\cdot s^{-5}$)", xlab = "Doppelschrittfrequenz ($1/min$)", pch = pchs, bg = color)
linear.model <- lm(data.1$mean.jerk.cost ~ data.1$mean.ms.interval)
abline(linear.model)
summary(linear.model)
grid(col = rgb(186/255, 187/255, 194/255))
box()

plotRelation(data.1$mean.hr, data.1$mean.jerk.cost, xlab = "Mittlere HR ($1/min$)", ylab = "Bew. ($\\times 10^3 \\: m^2 \\cdot s^{-5}$))", pch = pchs, bg = color)
grid(col = rgb(186/255, 187/255, 194/255))
box()

plotRelation(data.1$mean.hr, data.1$mean.nsei, ylab = "Norm. Shannon Entropie Index", xlab = "Mittlere HR ($1/min$)", pch = pchs, bg = color)
grid(col = rgb(186/255, 187/255, 194/255))
box()

plot(data.1$mean.hr, data.1$fit, ylab = "AFP", xlab = "Mittlere HR ($1/min$)", pch = pchs, bg = color)
linear.model <- lm(data.1$fit ~ data.1$mean.hr)
abline(linear.model)
summary(linear.model)
grid(col = rgb(186/255, 187/255, 194/255))
box()
...
```{r}
require(lattice)
xyplot(flow ~ mean.nsei, groups = groups, data = data.1, type = c("p", "r"), pch = c(21, 22, 23), xlab = "x", ylab = "Flow", auto.key = T)

summary(lm(data.1$flow ~ data.1$mean.hr + groups + data.1$mean.hr * groups)) 
...
plot(data.1$mean.hr, data.1$mean.nsei, xlab = "Mittlere HR (BPM)", ylab = "Absorbiertheit", pch = 21, bg = rgb(229/255, 66/255, 66/255))
abline(linear.model)
...
\begin{figure}
	...
	\caption{Beobachtungen zur Fallstudie zum Laufen. Mittlerer normalisierter Shannon Entropie Index und mittlerer HR. Quelle: Eigene Darstellung -+-\emph{Anmerkung}: Durchgezogene Linie stellt das bestmögliche liniare Modell dar. -+-Gestrichelte Linie stellt ggf. das bestmögliche quardratische Modell dar.}
	\label{fig:5_10_regression_extra}
\end{figure}


```{r}
median.cut.feature <- 15
test.feature <- 6

sub.1 <- data.1[data.1[,median.cut.feature] <= median(data.1[,median.cut.feature], na.rm = T),]
sub.2 <- data.1[data.1[,median.cut.feature] > median(data.1[,median.cut.feature], na.rm = T),]

t.test(sub.1[,test.feature], sub.2[,test.feature])

fac <- rep(1, 24) 
fac[c(5, 6, 8, 9, 10, 14, 18, 22, 23)] <- 2
fac[c(7, 12, 15, 16, 19, 20, 24)] <- 3

spm.d <- c(1:4, 11, 13, 17, 21)
balance <- c(5, 6, 8, 9, 10, 14, 18, 22, 23)
bpm.d <- c(7, 12, 15, 16, 19, 20, 24)

sub.1 <- data.1[spm.d,]
sub.2 <- data.1[balance,]
sub.3 <- data.1[bpm.d,]

plot(sub.2$mean.nsei, sub.2$flow, ylim = c(min(data.1$flow), 6), xlim = c(min(data.1$mean.nsei), max(data.1$mean.nsei)), pch = 21, bg = "cyan", xlab = "Phasensynchronisation", ylab = "Flow")
lmo <- lm(sub.2$flow ~ sub.2$mean.nsei)
abline(lmo, col = "cyan")
summary(lmo)

points(sub.1$mean.nsei, sub.1$flow, pch = 22, bg = "green")
lmo <- lm(sub.1$flow ~ sub.1$mean.nsei)
abline(lmo, col = "green", lty = "dashed")
points(sub.3$mean.nsei, sub.3$flow, pch = 23, bg = "red")
lmo <- lm(sub.3$flow ~ sub.3$mean.nsei)
abline(lmo, col = "red", lty = "dotted")

legend("topleft", pch = c(21,22,23), legend = c("Balance", "Schritt Dominanz (Unterf.)", "Herz Dominanz (Überf.)"), pt.bg = c("cyan", "green", "red"), text.width = 0.3)
