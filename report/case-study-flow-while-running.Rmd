...
```{r variables-session-wise, echo=FALSE, warning=FALSE, results="asis"}
  ...
  # Print latex table
  table                   <- xtable(feature.table, label = "tab:variablen_nach_sitzung_fallstudie_laufen", align = "lyyyyyyy", caption = "Variablen der Fallstudie zum Laufen: Arithmetisches Mittel $\\pm$ Standardabweichung zu den sechs Sitzungen [$N = 3$] -+- \\emph{Anmerkung}: Bew. = Bewegungsaufwand.") 
...
```{r box-and-whisker-plots, include=FALSE}
...
plotBoxAndWhiskerPlot(data.1$measurement, data.1$rmssd, col = rgb(0/255, 152/255, 199/255, .5), ylim = c(4, 12), main = "RMSSD", ylab = "$ms$", pch = 16)
...
plotBoxAndWhiskerPlot(data.1$measurement, data.1$mean.nsei, col = rgb(0/255, 152/255, 199/255, .5), ylim = c(0, .4), main = "Norm. Shan. Entr. Index", ylab = "$[0, 1]$", pch = 16)
