...
rm(e.seq, n.row, fss.features, cls.features, hrv.features, jc.features)
```
...
# Check group normality of features again
...
for(i in 1:length(data.3)) {
  ...
  print(feature.name)
  ComputeRepeatedAnova(date, measurement, data.3[, i]) 
}
...
# Use only data after the 30' mark
...
shapiro.test(data.3$fluency)
shapiro.test(data.3$absorption)
...
set.seed(2)
data.4 <- data.2
samples <- sample(1:nrow(data.4), 12, replace=F)
data.barbara <- data.4[samples, ]

rm(data.2, data.3, data.4, samples)
```

```{r process-orientation}

# Use only data after the 30' mark
m <- 2
data.2 <- cbind(data.1, date)
data.2 <- data.2[as.numeric(measurement) == m, ]

max.coh <- which.max(data.2$coh)
min.flow <- which.min(data.2$flow)
dates <- c(as.character(data.2[min.flow,]$date), as.character(data.2[max.coh,]$date))
...
rm(m, data.2, max.coh, min.flow, dates, time.range, ticks, i)
