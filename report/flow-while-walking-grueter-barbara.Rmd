...
for (i in 1:nrow(fss.features)) {
  ...
  if(file.exists(paste(directory.path, file.name, sep = ""))) {
    ...
    data[, 3]   <- data[, 3] / 10^4
    anomaly  <- DetectAnomaly(data[, 2], data[, 3], expression("Cycle Interval ("~s~")"), expression("JC (x"~10^4~m^2*s^{-5}~")"), c(.8, 1.3), c(0, 8), epsilon = epsilon) 
...
linear.model <- lm(fss.features$absorption ~ hrv.features$meanhr[breaks == 3])
...
linear.model <- lm(fss.features$absorption ~ hrv.features$rmssd[breaks == 3])
summary(linear.model)
plot(hrv.features$rmssd[breaks == 3], fss.features$absorption, pch = 21, cex = 1.2, ylab = absorption.expression, xlab = rmssd.expression)
abline(linear.model)
box()

linear.model <- lm(fss.features$absorption ~ (jc.features$mean.jerk.cost[breaks == 3]))
summary(linear.model)
