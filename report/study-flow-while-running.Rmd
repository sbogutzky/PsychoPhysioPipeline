...
fss.features <- data.1[-outliers, 2:8]
...
# Check normality 
for (i in c(2:4, 10, 13, 16, 18)) {
  print(colnames(data.1)[i])
  hist(data.1[-outliers, i], main = colnames(data.1)[i])
...
# Create APA table
apa.table.cor <- apa.cor.table(data.1[-outliers, c(2:4, 10, 13, 16, 18)], show.conf.interval = FALSE)
...
data.2$direction <- physiological.state
...
data.2 <- data.1[-outliers, c(2:4, 10:13, 15:18)]

print(length(data.2[physiological.state == 3, 1])) 
print(length(data.2[physiological.state == 2, 1])) 
print(length(data.2[physiological.state == 1, 1]))

for(i in 1:11) {
  par(mai = c(.5, .5, .2, .1), mgp = c(2.5, 1, 0), mfrow = c(1, 4))
  boxplot(data.2[, i] ~ factor(physiological.state, labels = c("Herz dominiert", "Gleichgewicht", "Schritt dominiert")), ylab = colnames(data.2)[i])
  
  print("---------------------")
  
  print(colnames(data.2)[i])
  
  hist(data.2[physiological.state == 3, i], main = colnames(data.2)[i]) 
  print(shapiro.test(data.2[physiological.state == 3, i]))
  print(mean(data.2[physiological.state == 3, i]))
  print(sd(data.2[physiological.state == 3, i]))
  
  hist(data.2[physiological.state == 2, i], main = colnames(data.2)[i]) 
  print(shapiro.test(data.2[physiological.state == 2, i]))
  print(mean(data.2[physiological.state == 2, i]))
  print(sd(data.2[physiological.state == 2, i]))
  
  hist(data.2[physiological.state == 1, i], main = colnames(data.2)[i]) 
  print(shapiro.test(data.2[physiological.state == 1, i]))
  print(mean(data.2[physiological.state == 1, i]))
  print(sd(data.2[physiological.state == 1, i]))

  print(var.test(data.2[physiological.state == 1, i], data.2[physiological.state == 2, i]))
  print(t.test(data.2[physiological.state == 1, i], data.2[physiological.state == 2, i], var.equal = T))
  print(var.test(data.2[physiological.state == 2, i], data.2[physiological.state == 3, i]))
  print(t.test(data.2[physiological.state == 2, i], data.2[physiological.state == 3, i], var.equal = T))
...
boxplot(data.2[, 1] ~ factor(physiological.state, labels = c("S", "G", "H")), ylab = "Generalfaktor", ylim = c(2.5, 7))
grid(col = rgb(186/255, 187/255, 194/255))
box()
mtext("A", side = 2, line = 3.5, padj = -7, las = 1)
boxplot(data.2[, 2] ~ factor(physiological.state, labels = c("S", "G", "H")), ylab = "Glatter Verlauf", ylim = c(2.5, 7))
grid(col = rgb(186/255, 187/255, 194/255))
box()
mtext("B", side = 2, line = 3.5, padj = -7, las = 1)
boxplot(data.2[, 3] ~ factor(physiological.state, labels = c("S", "G", "H")), ylab = "Absorbiertheit", ylim = c(2.5, 7))
grid(col = rgb(186/255, 187/255, 194/255))
box()
...
# Setup tikz device
#tikz("./tikz/prozessorientierter-ansicht_2.tex", width = 7.91, height = 5.22)

# Set graphic parameter
par(mai = c(.8, .8, .1, .1), mgp = c(2.5, 1, 0), mfcol = c(4, 3))
...
cls.phase.data.file.path <- paste(root.path, processed.data.directory, activity.directory, user.directory, date.directory, cls.phase.data.file.name, sep = "")
cls.index.data.file.path <- paste(root.path, processed.data.directory, activity.directory, user.directory, date.directory, cls.index.data.file.name, sep = "")
...
# Clean up
rm(cls.phase.data, jc.data, kubios.hrv.data, cls.phase.data.file.path, cls.phase.data.file.name, user.directory, date.directory, heart.beat.times, jc.data.file.path, jc.data.file.name, kubios.hrv.data.file.path, kubios.hrv.data.file.name, m, n, bpm.ylim, jc.ylim)
```
...
require(lattice)
summary(lm(data.2$flow ~ data.2$mean.hr + physiological.state + data.2$mean.hr * physiological.state))
xyplot(data.2$flow ~ data.2$mean.hr, groups = physiological.state, data = data.2, type = c('p','r'), grid = T, lwd = 3, auto.key = T)

summary(lm(data.2$flow ~ data.2$mean.stride.freq.right + physiological.state + data.2$mean.stride.freq.right * physiological.state))
xyplot(data.2$flow ~ data.2$mean.stride.freq.right, groups = physiological.state, data = data.2, type = c('p','r'), grid = T, lwd = 3, auto.key = T)
