...
for (u in 1:user.count) {
  ...
  # Create data frame
  data.1 <- rbind(data.1, data.frame(subject = subjects, fss.features[, c(1:7)], user.effort = user.efforts[u], hrv.features[, 1:2], cls.features[, 1:3], stride.freq.left = 60000 / mean.ms.interval.left, stride.freq.rigth = 60000 / mean.ms.interval.right, mean.jerk.cost.left / 10^3, mean.jerk.cost.right / 10^3, age))
}
...
data.3 <- data.3[, c(2:4,6,10,13,16,18,19)]
plot(data.3, col = physiological.state)

apa.cor.table(data.2, show.conf.interval = FALSE)

feature <- 3

boxplot(data.3[, feature] ~ physiological.state)
