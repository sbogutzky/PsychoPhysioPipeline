...
for (i in 1:participant.count) {
  ...
  # Create data frame
  data.1 <- rbind(data.1, data.frame(subject = subjects, fss.features[, 1:5], t(colMeans(cls.indexes[, 2:3]))))
  
}
rm(i, participant.dir, subjects, fss.feature.path, fss.features, cls.index.path, cls.indexes)
...
par("mfcol" = c(2, 1), mar = c(3, 3, 2.5, 1) + 0.1, mgp = c(2, .5, 0), las = 1, cex.axis = 0.8, tck = .03, cex.lab = .8, xaxs = "r", yaxs = "r")
